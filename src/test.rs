...
fn test() {
    ...
    let prop = Proposal {
        ...
        instr: vec![
            ...
            ProposalInstr {
                ...
                fun_name: symbol!("x_shares"), // Updated function name
                args: vec![
                    &env,
                    (10i32).into_val(&env),
    ...
    let prop = Proposal {
        ...
        instr: vec![
            ...
            ProposalInstr {
                ...
                fun_name: symbol!("xfer"), // Updated function name
                args: vec![
                    &env,
    ...
    token_client
        ...
        .mint(
            &Signature::Invoker,
            &BigInt::from_i32(&env, 0),
            &Identifier::Contract(dao_contract_id.clone()),
            &BigInt::from_u64(&env, 10),
        );
    assert_eq!(
        BigInt::from_u64(&env, 10),
        token_client.balance(&Identifier::Contract(dao_contract_id.clone()))
    );
    ...
    assert_eq!(
        BigInt::from_u64(&env, 9),
        token_client.balance(&Identifier::Account(user_1.clone()))
    );

    assert_eq!(
        BigInt::from_u64(&env, 1),
        token_client.balance(&Identifier::Contract(dao_contract_id.clone()))
    );
}
...
fn cannot_execute_prop_twice() {
    ...
    client.execute(&prop_id);
    client.execute(&prop_id); // Should panic
}
